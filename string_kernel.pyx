...
def string_kernel(n, lbda): # xs, ys
    
    def kernel(_xs, _ys):
        """xs and ys are numpy arrays of strings or arrays of ints, n an integer and lbda a bool"""
        if len(_xs.shape) != 2 or len(_ys.shape) != 2 or _xs.shape[1] != 1 or _ys.shape[1] != 1:
        ...
        lenxs, lenys = _xs.shape[0], _ys.shape[0]

        mat = np.zeros((lenxs, lenys))

        ssk_fun = ssk_array if _xs.dtype == 'O' and isinstance(_xs[0,0], array.array) else ssk

        # If both lists are equal, then the resulting matrix is symetric, there is no need to
        # calculate the hole thing
        if lenxs == lenys and np.array_equal(_xs, _ys):
            for i in range(lenxs):
                for j in range(i,lenys):
                    mat[j,i] = mat[i,j] = ssk_fun(_xs[i,0], _ys[j,0], n, lbda, accum=True)
            ...
            for i in range(lenxs):
                for j in range(lenys):
                    mat[i,j] = ssk_fun(_xs[i,0], _ys[j,0], n, lbda, accum=True)

            mat_xs = np.zeros((lenxs, 1))
            mat_ys = np.zeros((lenys, 1))

            for i in range(lenxs):
                mat_xs[i] = ssk_fun(_xs[i,0], _xs[i,0], n, lbda, accum=True)
            for j in range(lenys):
                mat_ys[j] = ssk_fun(_ys[j,0], _ys[j,0], n, lbda, accum=True)
